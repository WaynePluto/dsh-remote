...
- 每台挂上来的机器在入口机器上**分到一个固定端口**（如 `http://pc1:30810` 就是 pc2），可以直接存书签；入口机器配置 `relay.domain` 并前置 TLS 反代后，改用 `https://pc2.你的域名` 子域名访问（公网推荐，见[部署说明](deploy/README.md)）
...
在这台机器上打开 `http://127.0.0.1:30809` **免登录**（只有回环地址有这个待遇）。dsh 的地址不对外打印，一律走控制台——它会替你完成 dsh 0.1.2 起自带的 token 交换。退出：托盘菜单「退出」或终端 `Ctrl+C`，三个进程一起关掉。

## 把第二台机器挂上来

1. 在 **pc2** 上同样启动 dsh-remote，它会提示「没有远程入口」，正常，先挂着
2. 在 **pc1** 打开 `http://127.0.0.1:30809/_admin`，在「让另一台机器通过 pc1 开放」里填 pc2 的机器名，点签发，页面会给出一条可直接复制的命令：
   ```
...
>
> ⚠️ pc2 的 dsh 需要信任 pc1 的地址：粘完命令后**重启一次 pc2 的 dsh-remote**，启动器会自动带上。
