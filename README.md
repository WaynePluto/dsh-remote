...
终端会提示还没有管理员账号并给出一个地址。**在这台机器上**用浏览器打开它（如 `http://127.0.0.1:30809`）：
...
> 🔒 设置向导**只对 `127.0.0.1` 开放**，局域网里的其他人只会看到「请到那台机器上完成设置」，抢注不了管理员。
...
在这台机器上打开 `http://127.0.0.1:30809` **免登录**（只有回环地址有这个待遇）。dsh 的地址不对外打印，一律走控制台——它会替你完成 dsh 0.1.2 起自带的 token 交换。`Ctrl+C` 退出，三个进程一起关掉。
...
> 这些命令**不需要旧密码**——能在这台机器上执行命令的人本来就能直接读写数据库文件。真正要守住的是这台机器本身的登录。云服务器上设置向导只对 `127.0.0.1` 开放，本地浏览器够不着，用 `init` 创建管理员即可。
