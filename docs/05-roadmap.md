...
- [x] concise 与 concise-ptc 两个精简预设，PTC 复用官方工具执行链。
- [x] 固定 YOLO，用户提问保留人工回答。
- [ ] dsh 0.1.5-rc.2 适配；自动检查完成后仍需真实链路验收，复核入口见 [源码依据](02-dsh-facts.md)。
...
| turn-retry | 失败重试、停止继续、错误详情；有排队消息时不发送或修改队列 |
| services | 启动、就绪、日志、停止；重启 dsh 后服务存活并能认领 |
| terminal | 人类直接输入，忙碌时保留草稿，中断有效 |
