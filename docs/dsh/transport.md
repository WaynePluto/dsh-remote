...
仍回退到原有 HTTP/WS 传输。remote-settings 用 webserver/index-inject 的 global 行注入 true。
...
- connection 宿主半在认证与 Host/Origin 校验之后暴露 `connection/request` waterfall；
  当前 Agent 预设的目录打开走 dsh 原生能力，remote-settings 不接管预设目录请求。
