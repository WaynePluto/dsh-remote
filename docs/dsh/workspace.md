...
出处：`packages/client/ui-sidebar-documentpreview/src/client/image/{index.ts,ImageBody.tsx,ImageBody.module.css}`、
`src/client/document/contract.ts`（后三个 client 相对路径均属于同包）。

- 原生 ImageBody 按固有 CSS 像素尺寸显示，`width:auto`、`max-width:none`，无适应窗口、缩放、拖动控件。
...
- child owner 提供 `content`（原生已读取的完整 bytes）、`resourceAddress`、`wrap`、`scrollportRef`，
  并提供 `useTabInfo`。`scrollportRef` 可报告 renderer 自有滚动容器，传 null 退回共享正文滚动面；项目当前不接管这个 child owner。
- 原生图片内部负责 Blob URL 创建/回收和加载/失败状态，DOM 标记为 `data-image-preview`；SVG 仅作为
  img Blob URL 显示，不内联为活动 SVG。项目缩放增强只在原生 title slot 生成 portal 控件，并通过当前版本
  `data-textpreview-*`/`data-image-preview` 定位已渲染的 native image frame，写入 CSS zoom，不再读文件、复制字节缓存或改变 Blob/SVG 隔离。
- 该定位是版本受控局部契约；当前真实 dsh 已验证工具栏出现和 CSS zoom 变化，但深浅主题、不同尺寸图片、resize/卸载仍需矩阵验收。
...
- 图片缩放只在原生 text title slot 生成 document.body portal 控件，当前选中 image preview 的 frame 使用 CSS zoom；
