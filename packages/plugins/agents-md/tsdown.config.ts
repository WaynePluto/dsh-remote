...
const ID = '@dsh-remote/dsh-plugin-agents-md'
...
export default defineConfig([
  ...
  // exact service instances dsh is running. `dsh-home-paths` matters most here:
  // this plugin must resolve the SAME `$DSH_HOME` that dsh's own
  // `agent-instructions` row resolved, and a second bundled copy of that
  // resolver would be a second source of truth for the one path that matters.
  {
    name: `${ID}/host`,
    entry: ['src/index.ts'],
    format: 'esm',
