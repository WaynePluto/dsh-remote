...
2. 展开“交互终端”面板，在提示出现后自己输入并回车；输入不作为模型工具参数发送。
3. 需要停止前台命令时使用中断，发送固定 SIGINT。
...
仅需要持久 shell 状态均使用 pwsh/bash；可以改成非交互的优先改非交互。常驻服务用 services。

同一终端只允许一次发送，用户输入最多等待 10 秒；仍忙时保留草稿。
输入内容可能被目标程序回显或写入输出，输入不经模型参数并不等于输出中绝不会出现敏感信息。
...
终端输入框使用 dsh 主题字段（border-l4、bg-layer-1、brand focus、tertiary disabled）；终端输出区
仍是独立的等宽 code surface，不与输入框共用高度或边框。
