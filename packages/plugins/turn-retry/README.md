...
```powershell
pnpm --filter @dsh-station/dsh-plugin-turn-retry test
pnpm --filter @dsh-station/dsh-plugin-turn-retry typecheck
pnpm --filter @dsh-station/dsh-plugin-turn-retry build
node scripts/turn-retry-check.mjs
```

## 分发与管理

本包不作为独立安装项分发，而是 `@dsh-station/dsh-plugin-conversation-enhancements`（会话增强 Bundle）的组件。停用后，失败重试、停止继续横幅与排队消息保护消失；dsh 自带的自动重试不受影响。
