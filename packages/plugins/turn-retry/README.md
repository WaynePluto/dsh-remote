...
本插件只提供一个入口：输入框上方的持久重试栏。失败当场不再弹 `userQuestions`
询问卡片，也没有第二套重试交互。
...
供应商错误在投影里最多保留 2000 字，给模型的通知最多 300 字。错误详情放进
`max-height:7.5em` 的独立滚动容器；横幅同时复用 dsh 的 composer 宽度轴，
不会撑破消息区。
