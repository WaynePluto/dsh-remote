#!/bin/sh
# dsh-station 的 Linux / macOS 启动入口。
#
# 检查 Node.js 版本，然后从本脚本所在目录运行 dist/index.js。
# 多余的参数原样转交给 dsh-station，例如：./start.sh --config /etc/dsh-station.json
#
...
if ! command -v node >/dev/null 2>&1; then
  fail "[dsh-station] 没有找到 Node.js（命令 node 不存在）。" \
       "           dsh-station 使用你本机的 Node.js 运行，请到 ${NODE_URL} 下载安装 LTS 版" \
...
if [ -z "$VERSION" ]; then
  fail "[dsh-station] 认不出 Node.js 的版本号，node -v 输出的是：${RAW_VERSION}" \
...
if [ "$too_old" -eq 1 ]; then
  fail "[dsh-station] Node.js 版本太低：这台机器上是 v${MAJOR}.${MINOR}.${PATCH}，dsh-station 需要 ${MIN_VERSION} 或更高。" \
...
if [ ! -f dist/index.js ]; then
  fail "[dsh-station] 找不到 ${PACKAGE_ROOT}/dist/index.js。" \
       "           这个压缩包没有完整解压，请把整个 zip 重新解压一次。"
fi

printf '[dsh-station] Node v%s.%s.%s  (%s)\n' "$MAJOR" "$MINOR" "$PATCH" "$(command -v node)"
